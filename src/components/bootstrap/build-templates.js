...
fs.readdir("./templates-src", (err, files) => {
  ...
  files.forEach(file => {
    if (path.extname(file) === ".js" || path.extname(file) === ".css") {
      fs.createReadStream(`./templates-src/${file}`).pipe(
        fs.createWriteStream(`../../../public/templates/bootstrap/${file}`)
      );
      ...
      fs.readFile(`./templates-src/${file}`, "utf8", (err, data) => {
        ...
        fs.writeFile(
          `../../../public/templates/bootstrap/${file}`,
          template(data),
          err => {
            if (err) {
              return console.log(err);
            } else {
              console.log(`👍 [compiled] ./public/templates/bootstrap/${file}`);
            }
          }
        );
      });
