...
import { Menu, Dropdown, Button, Icon, Switch, Tooltip } from "antd";
...
class Header extends Component {
  ...
  handleImportMenuClick = async event => {
    const shouldImport = window.confirm(
      "Are you sure you want to import a new theme? Your theme variables will be overwriten."
    );
    if (shouldImport) {
      if (event.key.indexOf("bootswatch") === 0) {
        const themeName = event.key.replace("bootswatch/", "");
        const theme = await (
          await fetch(`./bootswatch/${themeName}.scss`)
        ).text();
  ...
  render() {
    ...
    let codeTooltipText = this.props.codeEditorOpen
      ? "Hide code snippets"
      : "Show code snippets";
    return (
      <header>
        <div className="header__logo">
          <img src="./images/makeui-logo.png" height="30" alt="makeui logo" />
        </div>
